...
const aiFeatures = [
  {
    ...
    description: "Open any syllabus topic and instantly generate detailed explanations, examples, revision notes, and key concepts.",
    icon: BookOpen,
    color: "bg-[#3B82F6]",
    ...
    chatData: {
      ...
      messages: [
        { type: "user" as const, text: "Show me the most important questions for Deadlocks from RGPV previous years." },
        { type: "ai" as const, text: "I've scanned the syllabus. Here are the most frequently asked questions on Deadlocks, sorted by weightage. I also generated a quick revision PDF for you:", uiComponent: "notes" as const }
      ],
      pyqs: [
  ...
  },
  {
    ...
    description: "Ask follow-up questions and receive personalized answers based on the topic you're studying.",
    icon: BrainCircuit,
    color: "bg-[#8B5CF6]",
    ...
    chatData: {
      ...
      messages: [
        { type: "user" as const, text: "I don't understand the difference between 2NF and 3NF. Explain it simply." },
        { type: "ai" as const, text: "Think of 2NF as 'the whole key' and 3NF as 'nothing but the key'." },
        { type: "user" as const, text: "Can you give me an example with a student database?" },
        { type: "ai" as const, text: "Sure! If an Employee table has 'Department_ID' and 'Department_Name'. Department_Name depends on Department_ID, not the Employee_ID. You should split them like this:", uiComponent: "tutor_code" as const },
      ]
    }
  },
  {
    title: "Exam Focused Answers",
    description: "AI-generated responses are optimized for engineering exam preparation and structured learning.",
    icon: Sparkles,
    color: "bg-[#10B981]",
    ...
    chatData: {
      ...
      messages: [
        { type: "user" as const, text: "Generate a 10-mark answer structure for 'OSI Reference Model'." },
        { type: "ai" as const, text: "Here is the optimal structure to score full marks in your exams:", uiComponent: "exam_structure" as const }
      ],
      pyqs: [
        "Explain the functions of all 7 layers of the OSI model. (10M)"
      ]
    }
  }
];
            ...
            >
              {aiFeatures.map((feature, idx) => {
                ...
                return (
                  ...
                  >
                    <div className={`absolute inset-0 -z-10 transition-opacity duration-500 ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                      <div className={`absolute -right-8 -top-8 h-24 w-24 rounded-full ${feature.color} opacity-5 dark:opacity-20 blur-2xl`} />
                    </div>
                    <div className="flex flex-col sm:flex-row items-start gap-3 sm:gap-4">
                      <div className={`inline-flex h-10 w-10 sm:h-10 sm:w-10 shrink-0 items-center justify-center rounded-[10px] sm:rounded-[12px] text-white shadow-sm ${feature.color} ${feature.shadow} dark:bg-gradient-to-br dark:from-white/10 dark:to-white/5 dark:shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)] dark:border dark:border-white/10 transition-transform duration-500 ${isActive ? 'scale-105' : 'group-hover:scale-110'}`}>
                        <Icon className="h-4 w-4 sm:h-4 sm:w-4" />
                      </div>
                      <div>
                        <h3 className={`text-[15px] sm:text-[16px] font-[800] tracking-tight transition-colors ${isActive ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-900 dark:text-white'}`}>
                          {feature.title}
                        </h3>
